module PPrintTH
#ifndef __FL2C__
   (Printable(..),Document,pprint,cleanpunct,
   nil,(!),(&),(!/),nl,sep,indented,indented',
   vpr,wpr,hpr,vmap,wmap,hmap,prsep,prpre)
#endif
  where
...
pprint x = fmt0 0 (apply (pr x) [])
  where
    ...
    -- The printer is design to avoid producing redundant spaces:
...
cleanpunct (P pr) = P (clean.pr)
  where
    clean os =
      case span (\o->isLayout o || isPunct o) os of
	([],[]) -> []
	([],o:os2) -> o:clean os2
        (os1,os2) ->
	  case partition isPunct os1 of
	    (os1a,os1b) -> os1a++os1b++clean os2

    isLayout (Str "\\end{itemize}") = True -- hmm
    isLayout (Str "\\end{quote}") = True -- hmm
    isLayout (Str _) = False
    isLayout _ = True

    isPunct (Str [c]) = c `elem` ",.:" -- more?
    isPunct _ = False
