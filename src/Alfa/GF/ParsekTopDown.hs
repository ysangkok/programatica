module ParsekTopDown (parsekParser) where
...
-- version with Koen Claessen's combinators 17/10/2000

-- parsekParser :: (Token a, CFIdent c) => CF a c -> CFCat c -> WParser a (CFTree c)
parsekParser cf@(rr,lit) cat tt = useParsek (pCfTree cf' cat) tt where
 cf' = (concat (map (predefRules lit) tt) ++ rr, lit)
...
pNet env net@(fin,start,trans) = foldr (<|>) status paths where
  status = if elem start fin then return [] else pzero
 ...
 (pNet env (fin,start,goodpaths) *?* makeCfTree env) >>= closure afterC
  where
   ...
   afterC x   = pNet env (fin,fromC,trans) >>=
                       (\y -> return (Left x : y) *?* makeCfTree env)
...
pSimple env@(_,cf) pi parser =
 case pi of
   NS cat -> pCfTree env cat  >>= (\x -> parser >>= (\y -> return (Left x:y)))
   TS str -> literOptTok str  >>= (\x -> parser >>= (\y -> return y))
   FS catfun ->                                            return [Right catfun]

--literOptTok :: Token a => RegExp a -> Parser a a
literOptTok (RegAlts [a]) | isZeroTok a  = return a
