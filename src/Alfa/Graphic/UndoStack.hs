module UndoStack2(UndoStack,undoStack,doit,undo,redo) where
--import Maybe
--import EitherUtils(mapMaybe)
--import Fudgets(mapMaybe)
--import Seq(seq)
...
instance Functor UndoStack where
  map f (U on u tu tr) = U on (map f u) (map f tu) (map f tr)
