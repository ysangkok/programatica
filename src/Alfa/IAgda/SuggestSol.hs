module SuggestSol(Tactic(..),solveC,suggestions,solveFirstOrder,suggestSafeSols) where 
...
solveFunApp1 ::  Constraint -> PCM [MetaSubst]
solveFunApp1 c = do 
...
solveFunApp2 :: Constraint -> PCM [MetaSubst]
solveFunApp2 c = 
...
solveFunApp3 ::  Constraint -> PCM MetaSubst
solveFunApp3 c = do 
    c' <- getVarAppSolve c
    solve1 c'

solveFunApp4 ::  Constraint -> PCM MetaSubst
solveFunApp4 c = do 
    c' <- getVarAppSolve c
    solve2 c'
...
solveProj1 ::  Constraint -> PCM MetaSubst
solveProj1 c = do 
    c' <- getProjSolve c
    solve1 c'
   

solveProj2 ::  Constraint -> PCM MetaSubst
solveProj2 c = do 
    c' <- getProjSolve c
    solve1 c'


solveC::  Constraint -> Tactic -> PCM [MetaSubst]
...
trySolveC ::  Constraint -> Tactic -> PCM [(MetaVar,Exp)]
trySolveC c t = do 
...
suggest1 ::  Constraint -> PCM [(MetaVar,Exp)]                
suggest1 c = 
...
suggestions :: [Constraint] -> PCM [(MetaVar,Exp)]
suggestions cs = 
...
getSolve (Constraint (EStop m (EClos env (EMetaV _ _ _ _))) v) = 
    return ((m,env),v)
getSolve (Constraint v (EStop m (EClos env (EMetaV _ _ _ _))) ) = 
    return ((m,env),v)
getSolve _ =  passError ENoSolve

{-
getEtaSolve :: Constraint -> PCM MCSol
getEtaSolve (Constraint v1 v2) = do
     (plain v1 v2 >>= (uncurry etasolve)) `handle_` (plain v2 v1  >>= (uncurry etasolve))
  where
  plain :: Value -> Value ->  PCM (MCApp,Value)
  plain lv rv  = split (appnormalized lv) where
      split ::  (Value, [(Bool, Value)]) -> PCM (MCApp,Value)
      split (EStop _ (EClos env (EMetaV m _ _ _)),vs) | null vs   = return (((m,env),[]),rv)
                                     | otherwise = return (((m,env),vs),rv)
      split _                        = passError ENoSolve

-}


buildMap      :: Environment -> Environment -> [UId] -> PCM [(UId,[UId])] 
-- The first environment is the environment in which the 
-- a meta variable is defined and the second is it's current environment
-- Is really defEnv needed? I think it is because of idata, but I'm not sure.

buildMap defEnv evalEnv xs = joinByFst `liftM` foldr bM (return []) xs where
          bM x r             = lookupEnv x  defEnv >>= ifxWas where
            ifxWas (EVar x0 _) = lookupEnv x0 evalEnv >>= ifxIs  where
              ifxIs (EVar  y _) = ((y,x0):) `liftM` r
              ifxIs _         = passError ENoSolve
            ifxWas _         = r


safeMap yxs = all(\ (y,xs)-> (isInternalUId y) || all(y==) xs) yxs
renameMap yxs = all (\(y,xs) -> length xs == 1)  yxs

suggestSimpleSafe :: Constraint -> PCM MetaSubst
suggestSimpleSafe c = do
    ((m, envc), vc)        <- getSolve c
    mI <- getMetaInfo m
    let cit = transInfo mI
        xs = freeVarScope $ fst cit
    yxs <- buildMap (environment mI) envc xs
    if safeMap yxs then valueToExp False True m vc >>=  
                           \e -> return (True,m,e) -- e is in scope, is on 
                                                   -- applicative form 
                                                   -- and contains no meta vars
        else passError ENoSolve


suggestSafeSols :: [Constraint] -> PCM ([Either MetaSubst MetaSubst])
suggestSafeSols cs = do
    (sols,remcs) <- foldM (suggestGen Left suggestSimpleSafe) ([],[]) cs
    (sols',remcs') <- foldM (suggestGen Right solve1) (sols,[]) remcs
    return sols'
    where suggestGen :: (MetaSubst -> Either MetaSubst MetaSubst) ->
                        (Constraint -> PCM MetaSubst) -> 
                        ([Either MetaSubst MetaSubst],[Constraint]) ->
                        Constraint -> PCM ([Either MetaSubst MetaSubst],[Constraint])
          suggestGen f solver (sols,cs) c = (do
               me <- solver c 
               return (f me:sols,cs)) `handle_` return (sols,c:cs)
...
--    CExpr
solve1, solve2 ::  Constraint -> PCM MetaSubst
solve_ :: Int ->  Constraint -> PCM MetaSubst
(solve1, solve2) = (solve_ 1,  solve_ 2)
solve_ tacnum c = do 
    -- traceM ("Solve : " ++ ppDebug c)
    ((m, envc), vc)        <- getEtaSolve c
    -- traceM ("Solve res: " ++ show m ++ " = " ++ ppDebug vc)
    mI <- getMetaInfo m
    let   ct = transInfo mI
          cit = fst ct 
          ...
          tac1 = do yxs <- buildMap (environment mI) envc xs
...
foldVars mI yxs v = go v where
  ...
  -- Variable names might be internal?: change!!
...
solveFirstOrder :: MetaVar -> Constraint -> PCM [MetaSubst]
...
getEtaSolve :: Constraint -> PCM MCSol
getEtaSolve (Constraint v1 v2)= plain    v1 v2 $ \ w1 ->
                             plain    v2 v1 $ \ w2 ->
                             etasolve w1 v2 $
