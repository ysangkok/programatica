...
exp10 =
      --trace "A" $
         l L_lam ..+ pB (pPArgsT' False) +.+ pExpr                              >>> Clam
...
pSummand = pBindId +.+ ( many pPArgsBT)                    >>> (,)

pSummand' :: CParser (Id, [CArg])
pSummand' = pBindId +.+ pPArgsBT                    >>> \i -> \a -> (i,[a])

pIndSummands :: CParser [(CSummand, [(Bool,CExpr)])]
pIndSummands = sepBy pIndSummand (l L_bar) 

pIndSummand :: CParser (CSummand, [(Bool,CExpr)])
pIndSummand = pSummand `into` \ (c,cas1) ->
              pIndSummandTyp `into` \ es ->
              succeed ((c,cas1), map ((,) False) es)
...
pClause1 i pi = dsm ..+ pClause i pi 
--
--pClauses :: Id -> CParser Id -> CParser [CClause]
--pClauses i pi = pClause i pi

pClause :: Id -> CParser Id -> CParser CClause
pClause i pi = piEq i pi ..+ eq ..+ pExpr              >>-  CClause
...
pPArgsT' :: Bool -> CParser ([Id],CExpr)
pPArgsT' b =  
       many1 pBindId   >>- (\xs -> (xs,CMeta (getIdPosition$ last xs) b (Just True) preMetaVar))   
      ||! pPArgsT
        
       


pPArgsBT :: CParser CArg
pPArgsBT = lp ..+ sepBy1 pBBindId cm +.+ dc ..+ pType +.. rp    >>> CArg 

pPArgs :: CParser CArgs
pPArgs = many pPArgsBT
...
pMeta b  = l L_uscore >>- (\p -> CMeta p b (Just True) preMetaVar)
           ||! l L_meta `into`(\p -> integer'        >>- CMeta p False (Just False)
                                   ||! succeed             (CMeta p b (Just False)  preMetaVar))

pOper :: CParser Id
pOper = pOpId ||! l L_bquote ..+ pVarId +.. l L_bquote

pModId, pVarId,pConId, pBindId, pOpId :: CParser Id
