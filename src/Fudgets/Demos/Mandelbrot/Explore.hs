...
image grays area = atomicD (FlexD 500 False False drawfunc)
  where
...
stripes sh dcmd@(CreatePutImage (Rect p (Point w h)) fmt pxls) =
  ...
  then [dcmd]
       ...
       in CreatePutImage (Rect p (Point w sh)) fmt pxls1 :
          stripes sh (CreatePutImage rect' fmt pxls2)

parallel [] = []
parallel xxs@(x:xs) = par x $ par (parallel xs) xxs
