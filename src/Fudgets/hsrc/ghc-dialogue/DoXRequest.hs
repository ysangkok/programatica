...
getGCValues = getValues newPtr getGCValue where
  getGCValue gcv ga = case ga of
    ...
    GCFont f               -> (SET(XGCValues,Int,gcv,font,toC f),CWORD32(GCFont))
    ...
    GCTile p               -> (SET(XGCValues,Int,gcv,tile,toC p),CWORD32(GCTile))
    GCStipple p            -> (SET(XGCValues,Int,gcv,stipple,toC p),CWORD32(GCStipple) :: Bitmask)
    -- _ -> (return (),0)
...
mkFontStructList' fs =
  do --putStrLn "Enter mkFontStructList'"
	 ...
	 # DEBUG(GET(XFontStruct,Int,fs,fid))
