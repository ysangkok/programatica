...
import IO(hPutStr,stderr) -- for error reporting
...
doXCommand (d,w,req) =
 ...
 where getdr = getdrawable w
	  ...
	  DrawArc (Rect (Point x y) (Point wi he)) a1 a2 ->
	    xDrawArc d drw gc x y wi he a1 a2
	  FillArc (Rect (Point x y) (Point wi he)) a1 a2 ->
	    xFillArc d drw gc x y wi he a1 a2
 ...
 where
  getWindowAttribute swa wa =
   case wa of
    ...
    CWCursor c		 -> (SETWa(swa,cursor,toC c),CWORD32(CWCursor))
    ...
    CWBackPixmap p       -> (SETWa(swa,background_pixmap,toC p),CWORD32(CWBackPixmap))
    CWBorderPixmap p     -> (SETWa(swa,border_pixmap,toC p),CWORD32(CWBorderPixmap))
