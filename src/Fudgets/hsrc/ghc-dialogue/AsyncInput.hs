...
--import qualified CString(CString)
--import PrelArr(newVar,readVar,writeVar,MutableVar)
--import Array
--import GhcWord
...
--		       _casm_ ``%r=FD_ISSET(%0,(fd_set*)%1);'' fd readfds
	  ...
	  return e
       ...
       mkEvent fd d = do
           (window,fev) <- getNextEvent display
...
foreign import ccall "sys/select.h select" cselect :: Int -> Cfd_set -> Cfd_set -> Cfd_set -> CtimeVal -> IO Int
foreign import ccall "unistd.h" getdtablesize :: IO Int

select :: Cfd_set -> Maybe CtimeVal -> IO Int
...
-}
    ...
    if (n :: Int) /= -1 then return n else do
       e <- errno
       if e == CCONST(EINTR) then start -- again
			 else return n
--}
...
doSocketRequest (XCallState  fds  aitable tq tno) sr =
 case sr of
   CreateTimer interval first -> do
	  ...
	  return (SocketResponse (Timer (Ti tnov)))
   DestroyTimer t -> do
          tqv <- readIOVar tq
...
--        _casm_ ``%r=stdin;''
   ...
   OpenFileAsSocket name mode -> do
     ...
     freePtr cname
     freePtr cmode
     return $ (SocketResponse . Socket . So) s
